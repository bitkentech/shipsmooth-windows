...
`[Local]` Script invocations use `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat <subcommand>`. All scripts read/write `.agents/plans/plan-{N}-tasks.xml` relative to the repo root.
...
ls .agents/plans/plan-{N}-tasks.xml   # if absent, run: %LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat init --plan {N} --tasks-from .agents/plans/plan-{N}.md

# 2. Review current task state
%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat show --plan {N}
...
`[Local]` All `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat` mutations also append one event to `.agents/ledger.jsonl` (content-addressed blobs in `.agents/objects/`). The XML remains the human-readable source of truth; the ledger is the machine-readable execution trace and the foundation for future parallel execution. Inspect with `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat ledger list` or `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat ledger verify`.
...
- `[Local]` Run `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat update-status --plan {N} --task {id} --status de-risked` and `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat add-comment --plan {N} --task {id} --message "De-risk draft ready for review"`.
...
- `[Local]` Run `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat update-status --plan {N} --task {id} --status agent-coded` and `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat set-commit --plan {N} --task {id} --commit $(git rev-parse HEAD)`.
...
4. Commit the completed task (tests + implementation):
   ...
   - `[Local]` Run `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat update-status --plan {N} --task {id} --status agent-coded` and `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat set-commit --plan {N} --task {id} --commit $(git rev-parse HEAD)`. No draft review needed.
...
- **Minor deviation** (task split, reorder, clarification):
  ...
  - `[Local]` Run `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat add-deviation --plan {N} --task {id} --type minor --message "..."`, continue.
- **Major deviation** (fundamental plan problem, architecture issue, blocked): Stop immediately.
  ...
  - `[Local]` Run `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat project-update --plan {N} --blocked --message "..."`.
...
- `[Local]` Run `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat project-update --plan {N} --status complete --message "Plan complete."`. Commit the final XML state. Update the permanent backlog feature issue (if tracked externally) or note delivery in the plan file.
...
- `[Local]` Run `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat update-status --plan {N} --task {id} --status needs-triage` for each unresolved task. Run `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat project-update --plan {N} --status in-review --message "..."`. Commit the XML. Wait for human to review.
...
- `[Local]` Run `%LOCALAPPDATA%\shipsmooth\0.3.11\runtime\bin\shipsmooth.bat project-update --plan {N} --status abandoned --message "Superseded by plan-{M}."`. Commit the final XML state.
